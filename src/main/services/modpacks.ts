...
async function importMrpack(
  ...
  if (index.dependencies.minecraft !== GAME.minecraftVersion) {
    throw new Error(
      `Сборка рассчитана на Minecraft ${index.dependencies.minecraft}, а Royale Master — на ${GAME.minecraftVersion}`,
    );
  }
...
async function exportMrpack(destination: string): Promise<ModpackResult> {
  ...
  const index: MrpackIndex = {
    ...
    name: "Royale Master",
    summary: "Сборка, экспортированная из Royale Launcher",
    files: indexFiles,
    dependencies: {
