...
import { ref } from "vue";
import type { InstalledMod, InstalledShaderPack, ModProject } from "@shared/types";
...
export const useShadersStore = defineStore("shaders", () => {
  ...
  const installed = ref<InstalledShaderPack[]>([]);
  ...
  function subscribe(): void {
    unsubscribe?.();
    unsubscribe = window.royale.shaders.onProgress((event) => {
  ...
  async function search(
    ...
    try {
      ...
      const response = await window.royale.shaders.search(
        query,
        category,
        sort,
        nextOffset,
  ...
  async function loadInstalled(): Promise<void> {
    installed.value = await window.royale.shaders.installedList();
  }
  ...
  async function loadProject(projectId: string): Promise<ModProject> {
    ...
    const project = await window.royale.shaders.project(projectId);
  ...
  async function install(project: ModProject): Promise<void> {
    ...
    error.value = "";
    try {
      await window.royale.shaders.installProject(project.project_id);
      await loadInstalled();
    } catch (reason) {
      error.value = cleanError(reason);
    } finally {
  ...
  async function remove(pack: InstalledMod): Promise<void> {
    await window.royale.shaders.remove(pack.filename);
    await loadInstalled();
  }

  async function removeMany(items: InstalledMod[]): Promise<void> {
    await Promise.all(
      items.map((item) => window.royale.shaders.remove(item.filename)),
    );
    await loadInstalled();
  }

  async function toggle(): Promise<void> {
    // Shader packs are selected from Minecraft's shader screen.
  }

  async function toggleMany(): Promise<void> {
    // Shared catalogue API: no file toggle is required for shader archives.
  }
