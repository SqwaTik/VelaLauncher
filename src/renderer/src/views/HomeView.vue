...
const instanceTitle = computed(() => {
  const words = (instances.active?.name || "Vela")
    .trim()
    .split(/\s+/);
...
const actionSecondary = computed(() => {
  ...
  if (launcher.updateInfo?.available)
    return `Vela ${launcher.updateInfo.remoteVersion}`;
  return "";
});
...
<template>
  <div class="home">
    ...
    <div
      ...
      role="img"
      aria-label="Интерактивный логотип Vela"
...
.default-visual::before {
  ...
  background: radial-gradient(
    circle,
    rgba(118, 104, 255, 0.2),
    rgba(67, 199, 244, 0.03) 54%,
    transparent 70%
  );
  filter: blur(12px);
}
...
.default-visual.dragging img {
  filter: drop-shadow(0 25px 42px rgba(118, 104, 255, 0.36));
}
.visual-orbit,
.visual-orbit i {
  position: absolute;
...
.visual-orbit i:nth-child(3) {
  inset: 28%;
  border-color: rgba(118, 104, 255, 0.32);
  box-shadow: inset 0 0 38px rgba(67, 199, 244, 0.08);
}
.visual-caption {
  position: absolute;
  right: -1%;
...
.gallery-card nav button.active {
  width: 24px;
  background: var(--green);
  box-shadow: 0 0 10px rgba(118, 104, 255, 0.7);
}
...
.launch-pill {
  ...
  box-shadow: 0 17px 42px rgba(88, 73, 220, 0.36);
  font-size: 17px;
  transition:
...
.launch-pill:has(.launch-main-action:hover:not(:disabled)) {
  ...
  box-shadow: 0 24px 58px rgba(88, 73, 220, 0.46);
...
.content-icons > b {
  ...
  flex: none;
  overflow: hidden;
  border: 2px solid rgba(20, 23, 38, 0.98);
  border-radius: 50%;
  color: var(--green);
  background: var(--surface-3);
